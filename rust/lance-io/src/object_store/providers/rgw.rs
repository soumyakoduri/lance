...
impl ObjectStoreProvider for RGWStoreProvider {
    async fn new_store(&self, base_path: Url, params: &ObjectStoreParams) -> Result<ObjectStore> {
        ...
        // Build RGW ObjectStore using object_store crate's RGW module
        #[cfg(feature = "rgw")]
        let inner_store = {
            use object_store::rgw::RGWObjectStoreBuilder;

            unsafe {
                RGWObjectStoreBuilder::new(self.driver, self.dpp)
                    .with_bucket(bucket_name.clone())
                    .build()
                    .map_err(|e| {
                        Error::invalid_input(format!("Failed to build RGW object store: {}", e))
                    })?
            }
        };
        ...
        let inner_store = {
            return Err(Error::invalid_input(
                "RGW support not enabled. Rebuild with --features rgw",
            ));
        };

        let mut inner: Arc<dyn OSObjectStore> = Arc::new(inner_store);
